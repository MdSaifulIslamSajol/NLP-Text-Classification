...
import pandas as pd
df = pd.read_csv("/home/saiful/bangla fault news/implicit hate speech detection/implicit-hate-corpus/implicit-hate-corpus/implicit_hate_v1_stg1_posts.tsv", delimiter='\t')
df



# Finding the unique classes in the class column
...
# # Load the dataset
# file_path = "/home/saiful/bangla fault news/hate speech detection/hate-speech-dataset/hate-speech-dataset-master/combined_data_with_text.csv"
# df = pd.read_csv(file_path)

# Define a dictionary for label mapping
label_map = {"implicit_hate": 1, "not_hate": 0,  "explicit_hate":2 }

# Map the labels to categorical values
df['label'] = df['class'].map(label_map)

# # Renaming the 'ID' column to 'text'
df.rename(columns={'post': 'text'}, inplace=True)

# # Display the updated dataframe
# print(df.head())
df.dropna(subset=['text', 'label'], inplace=True)
# Dropping all rows that contain labels with the value 2
...
#%%
# Separating the data based on labels
label_0_df = df_filtered[df_filtered['label'] == 0]
label_1_df = df_filtered[df_filtered['label'] == 1]

# Balancing the dataset by making the number of rows with label 1 equal to the number of rows with label 0
min_count = min(len(label_0_df), len(label_1_df))
balanced_label_0_df = label_0_df.sample(n=min_count, random_state=1)
balanced_label_1_df = label_1_df.sample(n=min_count, random_state=1)

# Concatenating the balanced dataframes
balanced_df = pd.concat([balanced_label_0_df, balanced_label_1_df])

# Shuffling the dataset to mix label 0 and label 1 rows
...
data = df
#%%
# Plotting the count of the labels
...
plt.xticks(ticks=range(len(label_map)), labels=label_map.keys(), rotation=0)
plt.show()
